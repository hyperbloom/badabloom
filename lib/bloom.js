 ...
 * Copyright (c) 2014-2017, Fedor Indutny (MIT License)
 * Copyright (c) 2014-2016, Christopher Jeffrey (MIT License).
 * https://github.com/indutny/bcoin
 */

function Bloom(size, n, tweak, filter) {
  if (filter) {
    this.filter = filter;
  } else {
...
Bloom.size = function size(filter) {
  return filter.length * 32;
};

Bloom.optimalParams = function optimalParams(count, prob) {
  const n = -Math.log(prob) / Math.log(2);
  return {
    size: Math.ceil(count * n / Math.log(2)),
    n: Math.round(n)
  };
};

Bloom.prototype.hash = function hash(val, n) {
  return murmur(val, sum32(mul32(n, 0xfa68676f), this.tweak)) % this.size;
};
